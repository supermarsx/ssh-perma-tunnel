...
// ---------------------------------------------------------------------------
// Local fork addition: real `gss_get_mic` / `gss_verify_mic` bindings.
//
// Upstream `libgssapi 0.9.1` only exposes `gss_wrap` / `gss_unwrap` via the
// `SecurityContext` trait. SSH `gssapi-with-mic` (RFC 4462 §3.5) requires a
// real `MIC` token (RFC 2743), which is wire-distinct from a non-encrypting
// `Wrap` token. The `libgssapi-sys` crate already generates the
// `gss_get_mic` / `gss_verify_mic` symbols via bindgen's `gss_.*` allowlist;
// only the high-level wrappers were missing. These two `unsafe fn`s and the
// matching `SecurityContext::{get_mic, verify_mic}` trait methods (added
// below) close that gap. See `.orchestration/logs/t7-P3.md`.
...
unsafe fn verify_mic(ctx: gss_ctx_id_t, msg: &[u8], mic: &[u8]) -> Result<(), Error> {
    let mut minor = GSS_S_COMPLETE;
    let mut msg = BufRef::from(msg);
    let mut tok = BufRef::from(mic);
...
pub trait SecurityContext {
    ...
    /// Compute a MIC (Message Integrity Code) token over `msg` via
    /// `gss_get_mic`. Unlike [`wrap`](Self::wrap), this produces a true
    /// RFC 2743 `MIC` token (wire-distinct from a non-encrypting `Wrap`
    /// token), as required by SSH `gssapi-with-mic` (RFC 4462 §3.5).
    ///
    /// Returned `Buf` contains only the MIC token; the caller is
    /// responsible for transmitting `msg` and the MIC separately.
    ///
    /// Local fork addition — not present upstream in `libgssapi 0.9.1`.
    fn get_mic(&mut self, msg: &[u8]) -> Result<Buf, Error>;

    /// Verify a MIC token `mic` against `msg` via `gss_verify_mic`.
    /// Returns `Ok(())` iff the underlying GSSAPI call returns
    /// `GSS_S_COMPLETE`. Errors propagate `gss_verify_mic`'s major/minor
    /// status (typically `GSS_S_BAD_MIC` for tampered tokens,
    /// `GSS_S_DEFECTIVE_TOKEN` for malformed input).
    ///
    /// Local fork addition — not present upstream in `libgssapi 0.9.1`.
