...
pub trait Signer: Sized + Send + 'static {
    type Error: From<crate::SendError> + Send + 'static;
    type Future: futures::Future<Output = (Self, Result<CryptoVec, Self::Error>)> + Send + 'static;
    ...
    for russh_keys::agent::client::AgentClient<R>
{
    ...
    type Future = std::pin::Pin<
        Box<dyn futures::Future<Output = (Self, Result<CryptoVec, Self::Error>)> + Send + 'static>,
    >;
    fn auth_publickey_sign(self, key: &key::PublicKey, to_sign: CryptoVec) -> Self::Future {
