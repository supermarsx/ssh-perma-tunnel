...
impl<H: Handler> Handle<H> {
    ...
    pub fn authenticate_future<'a, U: Into<String>, S: auth::Signer>(
        &'a mut self,
        user: U,
        key: key::PublicKey,
        future: S,
    ) -> std::pin::Pin<
        Box<dyn futures::Future<Output = (S, Result<bool, S::Error>)> + Send + 'a>,
    > {
        // t7-P1 (local-fork): clone the `Sender` and reborrow the `Receiver`
        // into locals up front so that the underlying `&mut self` is only
        // held for the duration of this function's preamble — the returned
        // boxed future captures `sender` (owned) and `receiver` (a `&'a mut`
        // reborrow that's already part of the `'a` outer lifetime). The
        // explicit `Send + 'a` annotation on the returned `dyn Future` then
        // commits the lifetime relationship at the trait boundary instead
        // of bubbling a higher-ranked-lifetime obligation through callers'
        // outer `Pin<Box<dyn Future + Send + 'static>>` coercions. The
        // matching `auth::Signer: Send + 'static` (with `Send + 'static`
        // associated types) carries this through the generic state machine.
        // See `.orchestration/logs/t7-P1.md`.
            ...
            loop {
                ...
                match reply {
                    ...
                    Some(Reply::SignRequest { key, data }) => {
                        ...
                        if sender.send(Msg::Signed { data }).await.is_err() {
                            return (future, Err((crate::SendError {}).into()));
                        }
                    }
