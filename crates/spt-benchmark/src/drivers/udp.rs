//! UDP echo driver (SSH3 only). Stubbed — needs `spt-ssh3` UDP capability,
//! wired in t1-e18.

use async_trait::async_trait;

use crate::driver::{BenchContext, BenchmarkDriver, ImpactLevel};
use crate::result::BenchResult;

/// Skip-by-default UDP driver.
#[derive(Default, Debug)]
pub struct UdpDriver;

#[async_trait]
impl BenchmarkDriver for UdpDriver {
    fn name(&self) -> &str {
        "udp"
    }
    fn impact(&self) -> ImpactLevel {
        ImpactLevel::Production
    }
    async fn run(&self, ctx: &BenchContext) -> BenchResult {
        BenchResult {
            driver: self.name().into(),
            iterations_attempted: ctx.iterations,
            payload_size: ctx.payload_size,
            errors: vec![
                "udp driver requires SSH3 datagram support; deferred to t1-e18".into(),
            ],
            env: ctx.env.clone(),
            started_at: chrono::Utc::now().to_rfc3339(),
            ..Default::default()
        }
    }
}
