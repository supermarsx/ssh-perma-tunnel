...
Register-ArgumentCompleter -Native -CommandName 'spt' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        ...
        'spt;ssh3-serve' {
            ...
            [CompletionResult]::new('--allow-target', '--allow-target', [CompletionResultType]::ParameterName, 'Allow-list of `host:port` forward targets the server will dial. May be repeated. Required unless `--fixed-target` or `--allow-open-relay` is supplied')
            ...
            [CompletionResult]::new('--require-authorization', '--require-authorization', [CompletionResultType]::ParameterName, 'Require this bearer/authorization value on the CONNECT request. When set, a CONNECT whose `Authorization` header does not match is rejected with HTTP 401. Prefer `--require-authorization-file` for production so secrets do not appear in process arguments')
            [CompletionResult]::new('--require-authorization-file', '--require-authorization-file', [CompletionResultType]::ParameterName, 'Read the required CONNECT `Authorization` header value from this file. Trailing CR/LF is ignored so newline-terminated secret files work')
            [CompletionResult]::new('--config', '--config', [CompletionResultType]::ParameterName, 'Path to a single config file')
            [CompletionResult]::new('--config-dir', '--config-dir', [CompletionResultType]::ParameterName, 'Path to a directory of `*.toml` configs (loaded in lexical order)')
            [CompletionResult]::new('--config-url', '--config-url', [CompletionResultType]::ParameterName, 'HTTPS URL of a remote config to fetch')
            [CompletionResult]::new('--config-fingerprint', '--config-fingerprint', [CompletionResultType]::ParameterName, 'SHA-256 fingerprint pin for `--config-url`')
            ...
            [CompletionResult]::new('--allow-open-relay', '--allow-open-relay', [CompletionResultType]::ParameterName, 'Explicitly allow peers to choose any `direct-tcp` target the server can reach. This creates an open TCP relay; prefer `--allow-target` or `--fixed-target` for production')
            [CompletionResult]::new('--portable', '--portable', [CompletionResultType]::ParameterName, 'Portable mode: keep all runtime state next to the executable instead of OS-standard locations (no OS install required)')
            [CompletionResult]::new('--json', '--json', [CompletionResultType]::ParameterName, 'Convenience alias for `--output json`')
            [CompletionResult]::new('-q', '-q', [CompletionResultType]::ParameterName, 'Suppress non-essential output')
            [CompletionResult]::new('--quiet', '--quiet', [CompletionResultType]::ParameterName, 'Suppress non-essential output')
