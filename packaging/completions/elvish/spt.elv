...
set edit:completion:arg-completer[spt] = {|@words|
    ...
    var completions = [
        ...
        &'spt;service;install'= {
            ...
            cand --run-as-user 'Run the service as this user (system scope). Maps to systemd `User=` / OpenRC `command_user` / launchd `UserName` / SysV `DAEMON_USER`'
            cand --run-as-group 'Run the service as this group (system scope). Maps to systemd `Group=`'
            cand --restart 'Restart policy for the generated unit'
            cand --watchdog-sec 'systemd `WatchdogSec=` interval in seconds. `0` disables the watchdog; omitted uses a sane default'
            cand --stdout 'Redirect service stdout to this path (launchd / SysV)'
            cand --stderr 'Redirect service stderr to this path (launchd / SysV)'
            cand --env 'Extra environment variable `KEY=VALUE` (repeatable)'
            cand --description 'Override the unit description'
            cand --config-dir 'Path to a directory of `*.toml` configs (loaded in lexical order)'
            cand --config-url 'HTTPS URL of a remote config to fetch'
            cand --config-fingerprint 'SHA-256 fingerprint pin for `--config-url`'
            cand --state-dir 'Override the runtime state directory'
            ...
            cand --sd-notify 'Enable systemd `Type=notify` (the daemon sends READY=1/STOPPING=1)'
            cand --portable 'Portable mode: keep all runtime state next to the executable instead of OS-standard locations (no OS install required)'
            cand --json 'Convenience alias for `--output json`'
            cand -q 'Suppress non-essential output'
            cand --quiet 'Suppress non-essential output'
        ...
        &'spt;firewall;apply'= {
            ...
            cand -y 'Confirm and perform live firewall mutation (required for apply outside --dry-run)'
            cand --yes 'Confirm and perform live firewall mutation (required for apply outside --dry-run)'
            cand --portable 'Portable mode: keep all runtime state next to the executable instead of OS-standard locations (no OS install required)'
            cand --json 'Convenience alias for `--output json`'
            cand -q 'Suppress non-essential output'
            cand --quiet 'Suppress non-essential output'
        ...
        &'spt;ssh3-serve'= {
            ...
            cand --allow-target 'Allow-list of `host:port` forward targets the server will dial. May be repeated. Required unless `--fixed-target` or `--allow-open-relay` is supplied'
            ...
            cand --require-authorization 'Require this bearer/authorization value on the CONNECT request. When set, a CONNECT whose `Authorization` header does not match is rejected with HTTP 401. Prefer `--require-authorization-file` for production so secrets do not appear in process arguments'
            cand --require-authorization-file 'Read the required CONNECT `Authorization` header value from this file. Trailing CR/LF is ignored so newline-terminated secret files work'
            cand --config 'Path to a single config file'
            cand --config-dir 'Path to a directory of `*.toml` configs (loaded in lexical order)'
            cand --config-url 'HTTPS URL of a remote config to fetch'
            cand --config-fingerprint 'SHA-256 fingerprint pin for `--config-url`'
            ...
            cand --allow-open-relay 'Explicitly allow peers to choose any `direct-tcp` target the server can reach. This creates an open TCP relay; prefer `--allow-target` or `--fixed-target` for production'
            cand --portable 'Portable mode: keep all runtime state next to the executable instead of OS-standard locations (no OS install required)'
            cand --json 'Convenience alias for `--output json`'
            cand -q 'Suppress non-essential output'
            cand --quiet 'Suppress non-essential output'
