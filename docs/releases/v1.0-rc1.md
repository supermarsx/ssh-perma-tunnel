# spt 1.0.0-rc1 — Release Notes

Tag candidate: `v1.0.0-rc1` (close-out of milestones t6 + t7 + t8).
Status: production-readiness audit closed; ship-blocking items all
resolved. See `docs/production_readiness.md` (v1.0) for the per-line
audit verdict.

This is the first release candidate for the 1.0 series. We expect to
promote to `v1.0.0` after a soak period if no regression is reported
against the gates listed in [Verification](#verification).

## Highlights

Three milestones land in this RC: **t6** (feature surface complete),
...
primitive is deferred to a post-1.0 operator decision. See *Known
...
*This document is a draft for the 1.0.0-rc1 tag. The shipped
release artifact will be regenerated from this template by the
`releasing.md` automation; any per-commit fix-ups between RC and GA
land in the `### Changed` section of `changelog.md` against
`[Unreleased]` first.*
