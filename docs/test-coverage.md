...
| SFTP | `spt-ssh2` + `spt-sftp` tests cover the russh SFTP client API, recursive put/get with resume + BPS cap + sha256, and the per-OS mount backends (Linux FUSE via `fuser`, Windows via Dokany2, macOS via `sshfs`). `spt-config` validates `[[profiles.sftp_mounts]]` capability gates, drive-letter policy, and cache modes. `spt-bin` tests cover mount/drive config mutation and planning surfaces. |
